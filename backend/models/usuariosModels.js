//var pool = require('./bd'); //llamada a la base de datos para consultas de datos
//var mssql = require('./bd');asi no 
//var poolConnect=require('./bd');
//var connectDB=require('./bd');
...
async function getUserByUsernameAndPassword(user, password) {
    try {
      // Esperamos a que el pool esté conectado, usando el poolconnect de bd.js uqe ya lo espera
      //lo cambie por que me tira error y no me deja logear a un usuario que si se encuntra en la base de datos
      //await poolConnect; 
      const pool = await connectDB();
      const hashedPassword = crypto.createHash('md5').update(password).digest('hex');//tengo que hacer esto para que compara por hashed 
      const result = await pool//ya que si no lo pongo compara texto con hash
        .request()
        .input('usuario', mssql.VarChar, user)
        .input('password', mssql.VarChar, hashedPassword)
        .query('SELECT * FROM USUARIOSTRANSX WHERE usuario = @usuario AND password = @password');
